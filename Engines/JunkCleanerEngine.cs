...
public class JunkCleanerEngine
{
    ...
    private (long bytes, long cleanableBytes, long lockedBytes, int count, List<JunkFileItem> files) GetDirectoryDetails(string path, string searchPattern = "*", bool recursive = true, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (recursive)
            {
                ...
                foreach (var dir in dirs)
                {
                    ...
                    var (subBytes, subCleanable, subLocked, subCount, subFiles) = GetDirectoryDetails(dir, searchPattern, recursive, token);
                    bytes += subBytes;
