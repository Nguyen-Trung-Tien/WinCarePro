...
public class JunkCleanerEngine
{
    ...
    public async Task<List<JunkCategory>> ScanJunkAsync()
    {
        return await Task.Run(() =>
        {
            ...
            {
                Name = Services.TranslationManager.Instance.T("Web Browser Caches"),
                Description = Services.TranslationManager.Instance.T("Cached web pages, scripts, and media files from Edge, Chrome, and Brave."),
            ...
            categories.Add(new JunkCategory
            {
                Name = Services.TranslationManager.Instance.T("DirectX Shader Cache"),
                Description = Services.TranslationManager.Instance.T("Graphics processor shader cache. Speed up application load times, but safe to clear."),
            ...
            categories.Add(new JunkCategory
            {
                Name = Services.TranslationManager.Instance.T("System Log Files & Error Reports"),
                Description = Services.TranslationManager.Instance.T("Activity logs, crash dump registers, and error reports generated by Windows."),
    ...
    private (long bytes, int count) GetDirectorySize(string path)
    {
        ...
        try
        {
            ...
            {
                var (subBytes, subCount) = GetDirectorySize(dir);
                bytes += subBytes;
                count += subCount;
            }
        }
