...
public partial class DashboardViewModel : ViewModelBase, IDisposable
{
    ...
    // Fix Bug 3: Dùng static Random thay vì new Random() mỗi lần gọi
    ...
    private (double cpu, double ramPercent) GetSystemResourceUsage()
    {
        ...
        if (!cpuReadSuccess)
        {
            // Fix Bug 3: dùng static _rand instance
        ...
        Task.Run(async () =>
        {
            ...
            try
            {
                while (!token.IsCancellationRequested)
                {
                    ...
                    // Fix Bug 5: Uptime chỉ update mỗi 30 tick (~30s) thay vì mỗi giây
                    bool shouldUpdateUptime = tickCount % 30 == 0 || tickCount == 1;

                    // Fix Bug 4: Batch dispatch CPU/RAM update + chart vào 1 lần enqueue
                    _dispatcherQueue?.TryEnqueue(() =>
                    {
                        if (token.IsCancellationRequested) return;
                        ...
                        // Update chart collections (họ maintain 30 points rolling)
                        ...
                        // Fix Bug 5: chỉ update uptime khi cần (30s interval)
    ...
    {
        _monitorCts?.Cancel();
        // Cancel scan nếu đang chạy – giải phóng scan khi navigate away
        CancelScanIfRunning();
    }
    ...
    public async Task RunFullDiagnosticsAsync()
    {
        ...
        {
            // Scan bị cancel: do navigate away hoặc timeout 90 giây
            _dispatcherQueue?.TryEnqueue(() =>
            {
        ...
        {
            // Luôn đảm bảo IsScanning = false và giải phóng _scanCts sau khi scan kết thúc
            // (phòng trường hợp exception không được catch)
            _dispatcherQueue?.TryEnqueue(() =>
            {
                if (IsScanning)
                {
