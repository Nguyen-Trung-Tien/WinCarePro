...
public class NetworkViewModel : ViewModelBase
{
    ...
    {
        // Remove adapters that no longer exist
        var newNames = new HashSet<string>(newList.Select(a => a.Name));
        for (int i = Adapters.Count - 1; i >= 0; i--)
        {
            if (!newNames.Contains(Adapters[i].Name))
            {
                Adapters.RemoveAt(i);
            }
        }

        // Update or insert adapters
        for (int i = 0; i < newList.Count; i++)
        {
            ...
            {
                if (Adapters[j].Name == newItem.Name)
                {
                    existingIndex = j;
                    break;
                }
            ...
            if (existingIndex >= 0)
            {
                var existingItem = Adapters[existingIndex];
                bool changed = existingItem.Description != newItem.Description ||
                               existingItem.Status != newItem.Status ||
                               existingItem.Speed != newItem.Speed ||
                ...
                if (changed)
                {
                    Adapters[existingIndex] = newItem;
                }
                else if (existingIndex != i)
                {
                    Adapters.Move(existingIndex, i);
                }
            }
            else
            {
                Adapters.Insert(i, newItem);
            }
        }
    }
        ...
        try
        {
            _dispatcherQueue?.TryEnqueue(() =>
            {
                try
                {
                    ...
                    // In-place sync Connections
                    var newKeys = new HashSet<string>(filtered.Select(c => $"{c.Protocol}_{c.LocalAddress}_{c.ForeignAddress}_{c.Pid}"));
                    
                    for (int i = Connections.Count - 1; i >= 0; i--)
                    {
                        var key = $"{Connections[i].Protocol}_{Connections[i].LocalAddress}_{Connections[i].ForeignAddress}_{Connections[i].Pid}";
                    ...
                    for (int i = 0; i < filtered.Count; i++)
                    {
                        ...
                        var newKey = $"{newItem.Protocol}_{newItem.LocalAddress}_{newItem.ForeignAddress}_{newItem.Pid}";
                        
                        int existingIndex = -1;
                        for (int j = 0; j < Connections.Count; j++)
                        {
                            var existingKey = $"{Connections[j].Protocol}_{Connections[j].LocalAddress}_{Connections[j].ForeignAddress}_{Connections[j].Pid}";
                        ...
                        if (existingIndex >= 0)
                        {
                            var existingItem = Connections[existingIndex];
                            bool changed = existingItem.State != newItem.State || 
                                           existingItem.ProcessName != newItem.ProcessName;

                            if (changed)
                            {
                                Connections[existingIndex] = newItem;
                            }
                            else if (existingIndex != i)
                            {
                                Connections.Move(existingIndex, i);
                            }
                        }
                        else
                        {
                            Connections.Insert(i, newItem);
                        }
                    }
                }
                catch { }
