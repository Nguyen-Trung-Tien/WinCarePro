...
- Thư mục hệ thống: `C:\Windows`, `C:\Windows\System32`, `C:\Windows\SysWOW64`, `C:\Windows\WinSxS`, `C:\Program Files`, `C:\Program Files (x86)`
...
## 🛡️ 3. Bảo Vệ Dịch Vụ Cốt Lõi Windows (`ServiceSafetyService`)

1. **Danh sách trắng bảo vệ (Service Whitelist):**
   - Nghiêm cấm vô hiệu hóa hoặc dừng các dịch vụ thiết yếu: `RpcSs`, `DcomLaunch`, `SamSs`, `gpsvc`, `ProfSvc`, `BFE`, `WinDefend`, `CryptSvc`, `EventLog`, `PlugPlay`.
2. **Kiểm tra trước khi thay đổi trạng thái:**
   - Mọi thao tác tinh chỉnh trong `StartupServicesEngine` hoặc `SystemOptimizerEngine` phải gọi `ServiceSafetyService.IsProtectedService(serviceName)`. Nếu là dịch vụ được bảo vệ, phải từ chối hành động và trả về cảnh báo an toàn.
